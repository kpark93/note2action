...
export function HistoryRow({ item, onOpen }: HistoryRowProps) {
  ...
  return (
    <div
      ...
      // Only the row itself — Enter on the Reopen button must not open the modal.
      onKeyDown={(e) => {
        if (e.key === "Enter" && e.target === e.currentTarget) onOpen(item.id);
      }}
      className={`history-row grid ${ROW_COLS} cursor-pointer items-center gap-3 rounded-[13px] bg-card px-4 py-[9px]`}
    >
