...
describe("taskRows", () => {
  ...
  it("shows only saved, still-open items", () => {
    const ids = taskRows(all, "All", "All").map((r) => r.id);
  ...
  it("filters by owner and status together", () => {
    expect(taskRows(all, "Kyle Park", "Blocked").map((r) => r.id)).toEqual([
      kyleBlocked.id,
    ]);
    expect(taskRows(all, "Rachel Ng", "Blocked")).toHaveLength(0);
  });

  it("derives the presentation fields", () => {
    const [first, , third] = taskRows(all, "All", "All");
